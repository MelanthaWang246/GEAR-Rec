...
path = "D:/Desktop/KGCN-colakg-semantic/data/ml-1m" # your data path
...
orig_to_mapped = {}
with open(path + '/item_map.txt', 'r') as f:
    for line in f:
        orig_id, mapped_id = line.strip().split()
        orig_to_mapped[int(orig_id)] = int(mapped_id)

question_dic = {}
for movie_id, name in movie_id_name_map.items():
    if movie_id not in orig_to_mapped:
        continue
    mapped_id = orig_to_mapped[movie_id]
...
with open('D:/Desktop/KGCN-colakg-semantic/data/ml-1m/llm_input_item.json', 'w') as f:
